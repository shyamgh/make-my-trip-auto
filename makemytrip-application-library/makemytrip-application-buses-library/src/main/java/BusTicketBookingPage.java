...
public class BusTicketBookingPage extends AbstractPage implements ElementLocators {
	...
	public BusTicketBookingPage(WebDriver DRIVER) {
		...
		initPageObject();
		/*
		 * // Check that we're on the right page. if
		 * (!"MakeMyTrip".equals(driver.getTitle())) { // Alternatively, we
		 * could navigate to the login page, perhaps logging out first throw new
		 * IllegalStateException("This is not the home page"); }
		 */
	}
	...
	protected void initPageObject() {
		byDepartureCity = By.xpath(BUSTICKETBOOKING_SOURCE_CITY_TXT_XPATH);
		byDestinationCity = By.xpath(BUSTICKETBOOKING_DESTINATION_CITY_TXT_XPATH);
		byEmail = By.name(BUSTICKETBOOKING_EMAIL_TXT_XPATH);
		byDatepicker = By.xpath(BUSTICKETBOOKING_DATEPICKER_XPATH);
	}
	...
	public BusTicketBookingPage ClickThisPageLink(String tabName) {
		...
		CustomLogger.logMessage(Level.INFO, "Verify page link", "Verifying if the tab [" + tabName + "] is displayed",
				1);
	...
	public WebElement getThisWidgetTab(String tabName) {
		String id = "header_tab_" + tabName.toLowerCase();
		CustomLogger.logMethodStart();
		CustomLogger.logMessage(Level.INFO, "Verify widget tab displayed",
				"Verifying if the tab [" + tabName + "] is displayed", 1);
		Assert.assertTrue(DRIVER.findElement(By.id(id)).isDisplayed(),
				"Verify widget tab [" + tabName + "] is displayed");
		CustomLogger.logMethodEnd();
	...
	/**
	 * This method is used to enter departure and destination cities for
	 * searching bus
	 * 
	 * @param depCity
	 * @param destCity
	...
	public BusTicketBookingPage SelectToAndFrom(String depCity, String destCity) {
		...
		CustomLogger.logMessage(Level.INFO, "Verify element displayed",
				"Verifying if text box [" + byDepartureCity + "] is displayed", 1);
		...
		a.moveToElement(departureCity).click().build().perform();
		try {
			Thread.sleep(500);
		} catch (InterruptedException e) {
		}

		a.sendKeys(depCity).build().perform();
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e) {
		}
		...
		CustomLogger.logMessage(Level.INFO, "Verify element displayed",
				"Verifying if text box [" + byDestinationCity + "] is displayed", 1);
		Assert.assertTrue(genFun.verifyElementPresent(byDestinationCity),
				"Verify destination city text box is displayed");
		destinationCity = DRIVER.findElement(byDestinationCity);
		CustomLogger.logMessage(Level.INFO, "Enter text", "Enter text [" + destCity + "]", 1);
		a.moveToElement(destinationCity).click().build().perform();
		try {
			Thread.sleep(500);
		} catch (InterruptedException e) {
		}

		a.sendKeys(destCity).build().perform();
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e) {
			e.printStackTrace();
	 ...
	 * @author shyam
	 * @param currMonth
	 *            : if the date from current month
	 * @param date
	 *            : date of journey
	 */
	public BusTicketBookingPage SelectDateOfJourney(boolean currMonth, String date) {
		...
		wb = null;
		if (currMonth)
			wb = genFun.waitForElement(By.xpath(BUSTICKETBOOKING_DATEPICKER_CURRMONTH_XPATH + "//a[.='" + date + "']"));
		else
			wb = genFun.waitForElement(By.xpath(BUSTICKETBOOKING_DATEPICKER_NEXTMONTH_XPATH + "//a[.='" + date + "']"));
		wb.click();
		CustomLogger.logMethodEnd();
		return this;
	}
	...
	public BusTicketBookingPage EnterEmail(String eml) {
		CustomLogger.logMethodStart();
		CustomLogger.logMessage(Level.INFO, "Verify element displayed",
				"Verifying text box [" + byEmail + "] is displayed", 1);
	...
	public void SubmitSearch() {
		...
		Assert.assertTrue(genFun.verifyElementPresent(By.id("searchBtn")), "Verify submit button is displayed");
	...
	public void HandleSpecialSearchPopup() {
		...
		CustomLogger.logMessage(Level.INFO, "Verify element displayed",
				"Verifying [" + element.toString() + "] is displayed", 1);
		Assert.assertTrue(genFun.verifyElementPresent(By.name("submit_name")),
				"Verifying [" + element.toString() + "] is displayed");
